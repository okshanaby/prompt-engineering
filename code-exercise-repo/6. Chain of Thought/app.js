...
function buildExportPayload() {
  ...
  const modelCounts = {};
  prompts.forEach(p => {
    const m = p.metadata?.model;
    if (m) modelCounts[m] = (modelCounts[m] || 0) + 1;
  });
  const mostUsedModel = Object.keys(modelCounts).sort((a, b) => modelCounts[b] - modelCounts[a])[0] ?? null;
...
function validateImportPayload(data) {
  ...
  if (data.version !== EXPORT_VERSION) throw new Error(`Unsupported version: ${data.version}. Expected ${EXPORT_VERSION}.`);
  if (!Array.isArray(data.prompts)) throw new Error('Invalid file: "prompts" must be an array.');
  if (typeof data.notes !== 'object' || Array.isArray(data.notes)) throw new Error('Invalid file: "notes" must be an object.');

  data.prompts.forEach((p, i) => {
    if (typeof p.id === 'undefined') throw new Error(`Prompt at index ${i} is missing an id.`);
    if (typeof p.title !== 'string' || !p.title.trim()) throw new Error(`Prompt at index ${i} has an invalid title.`);
    if (typeof p.content !== 'string') throw new Error(`Prompt at index ${i} has invalid content.`);
  });
}
...
document.getElementById('import-input').addEventListener('change', (e) => {
  ...
  reader.onload = (ev) => {
    ...
    const duplicates = existing.filter(p => incomingIds.has(p.id));
    ...
    let summary = `Importing ${data.prompts.length} prompt(s).`;
    if (duplicates.length) {
      summary += ` ${duplicates.length} duplicate ID(s) detected — choose how to proceed:`;
      document.getElementById('modal-merge').textContent = `Merge (skip ${duplicates.length} duplicate${duplicates.length > 1 ? 's' : ''})`;
    } else {
      summary += ' No duplicates found. Choose how to proceed:';
      document.getElementById('modal-merge').textContent = 'Merge';
    }
...
function applyImport(mode) {
  ...
  const backupPrompts = localStorage.getItem(STORAGE_KEY);
  const backupNotes = localStorage.getItem(NOTES_KEY);
  ...
  try {
    if (mode === 'replace') {
      ...
      savePrompts([...existing, ...newPrompts]);

      const existingNotes = loadAllNotes();
      const mergedNotes = { ...data.notes, ...existingNotes };
      localStorage.setItem(NOTES_KEY, JSON.stringify(mergedNotes));
    }
    renderCards();
  } catch (err) {
    if (backupPrompts !== null) localStorage.setItem(STORAGE_KEY, backupPrompts);
    else localStorage.removeItem(STORAGE_KEY);
    if (backupNotes !== null) localStorage.setItem(NOTES_KEY, backupNotes);
    else localStorage.removeItem(NOTES_KEY);
    alert(`Import failed and was rolled back: ${err.message}`);
  }
}

document.getElementById('modal-merge').addEventListener('click', () => applyImport('merge'));
document.getElementById('modal-replace').addEventListener('click', () => {
  if (confirm('This will delete all existing prompts and replace them with the imported data. Continue?')) {
    applyImport('replace');
  }
});
