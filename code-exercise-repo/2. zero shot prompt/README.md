# Zero-Shot Prompt — Exercise

## Technique

**Zero-Shot Prompt** — a prompt that gives the model a task with no examples, demonstrations, or prior context. The model relies entirely on its pre-trained knowledge to complete the request.

## Prompt Used

> Create a prompt library application in HTML, CSS, and JavaScript.
>
> Create an HTML page with a form containing fields for the prompt title and content.
>
> Add a save prompt button that saves to localStorage.
>
> Display saved prompts in cards.
>
> Each prompt card should show the title, a content preview of a few words, and a delete button.
>
> Deleting should remove the prompt from localStorage and update the display.
>
> Style it with CSS to look clean and modern with a developer theme.
>
> Include all HTML, CSS, and JavaScript in separate files with no extra features.

## Model

**Claude Sonnet 4.6** (`claude-sonnet-4-6`)

## Effort

**LOW** (Minimal) — no follow-up iterations or corrections needed. The model produced a complete, working application in separate files from a single zero-shot instruction.
